...
public class Mr231_TTMController implements Initializable {
    ...
    protected void onSendButtonClick() {
        try {
            ...
            PreparedStatement st = App.db.getConnection()
                    .prepareStatement("insert into tb_ttm values (?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            /*PreparedStatement st = App.db
                    .prepareStatement("insert into tb_ttm values (?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?)");*/
            st.setTimestamp(1, Timestamp.valueOf(recTime.getText()));
            st.setLong(2, Timestamp.valueOf(recTime.getText()).getTime());
    ...
    private void UpdateMessages() {
        ...
        try {
            ...
            ResultSet rs = st.executeQuery("select * from tb_ttm where msgtype = 1");
            while (rs.next()) {
